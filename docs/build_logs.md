...
## 2026-07-20 - Creating an LED Blink Test and Troubleshooting Virtual Machine USB Connectivity
- Today I have so far completed writing two versions of the LED blinking light test in Arduino C++. Both can be found in ~/code/legacy/led_tests. The goal with these was to establish a connection with the pico board through USB and test whether or not I can upload my code onto the pico and have it function as intended.
- The code files both work as intended and serve slightly different purposes, but are largely interchangable for the sole purpose of testing the onboard LED of the pico. 
- Some troubleshooting issues I ran into is that PlatformIO's process of Build -> Upload -> Serial_Monitor has the USB identification change in each step, causing disconnection hiccups in my virtual machine. To fix this, in the VM's settings->USB setup, I created a USB filter for my pico and then got rid of every identifier besides Vendor ID and Name. Product ID through Port were all left blank. This allowed the VM to instantly capture the pico as a device even though PlatformIO changes its identity slightly with each action it takes (Build, Upload, etc.). 
- The next step I need to take is to verify that the Robo Pico connects as it should and double check that all the pins work as they should and are actually the same as the product's diagram claims.
